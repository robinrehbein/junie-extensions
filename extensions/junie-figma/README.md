...
This is the workaround for the remote Figma MCP (`https://mcp.figma.com/mcp`) refusing Junie at OAuth
(`DCR failed with 403 Forbidden` — Junie isn't on Figma's client allowlist of Cursor / VS Code /
Claude Code / Codex / Xcode). A PAT has **no** client allowlist, so it works where the remote server
403s.
...
1. **Create a Figma Personal Access Token** (Settings → Account → Personal access tokens) and register
   the MCP server. Full steps are in the [server README](../../servers/figma-mcp/README.md#one-time-setup).
   In short, add to `~/.junie/mcp/mcp.json` (or `.junie/mcp/mcp.json`):
...
> Implement this screen in Compose: https://www.figma.com/design/oZbZewuJDrJLXBBKgXLKVb/Noah?node-id=1123-747
