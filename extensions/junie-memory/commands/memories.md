...
1. Read `~/.junie/memory/MEMORY.md` and present the memories grouped by `type`
   (`user` / `feedback` / `reference`). If the store is empty or missing, say so and stop.
   Note: durable project facts live in the knowledge store (`junie-knowledge`), not here.
