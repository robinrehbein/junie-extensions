...
Drive a **self-hosted Plane** issue/ticket tracker from Junie CLI via a thin `scripts/plane.sh`
helper (plain `curl` + `jq`). No MCP server — the old Plane MCP was broken in this setup, so the
helper does the REST calls directly.
...
The skill calls `scripts/plane.sh`, i.e. a script expected at the **repo's** `scripts/` root
...
- **No MCP.** MCPs don't currently work in this setup; a `curl`+`jq` helper is simpler, debuggable,
  and dependency-light (`curl`, `jq`).
- **Client-side filtering.** Self-hosted Plane ignores list filters server-side, so list/issue
  lookups fetch then filter with `jq` (mirrors the old MCP behaviour).
- **Noah-specific defaults.** The bundled `plane.sh` ships the Noah workspace slug, project UUIDs,
  and member names for readable output. For another instance, edit the `PROJ`/`USER` maps and
