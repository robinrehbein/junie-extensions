...
- **Enforced by construction:** `reviewer` is read-only because its tool list has no
  Bash — it cannot run commands at all; the main agent supplies the diff in the brief.
- **Prompt-level only:** `implementer` and `test-runner` keep Bash (`test-runner` needs
  it to run tests); their rules — no `git commit`, no destructive git, no pushing, no
  installs, no network-mutating commands — are instructions, not enforcement. Under
  `braveMode: "ON"` nothing stops a confused agent from ignoring them.

For unattended `braveMode: "ON"` sessions, the lever is Junie's **Action Allowlist** (or
keep command approval on) — user-level enforcement an extension cannot ship.
